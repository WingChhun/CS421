...
bool contains_isNotSubject(string lineFromFile) {
	...
	if (temp != -1) {
		return true;
	} //false, there is no subject stirng
	return false;
}
...
int main() {
	...
	if (!file) {
		...
		while (!file.eof()) {
			...
			if (contains_isStartOfEmail(lineFromFile)) {} // DO NOTHING
			else {
				...
				else if (contains_isNotSubject(lineFromFile)) {} //DO NOTHING, IGNORE SUBJECT LINE
				//Final Check - is line </DOC> closing XML tag
					...
					if (dfaState != state_final) { //Once FA reaches state_final, FA stays there, so this will hold and we can determine to pop or push the ID
...
void printStateTransition(enum State currentState, char c) {
	cout << "Transition( q" << currentState << ", " << c << " ) =  "; //to be finished within state Function

}
//FINITE AUTOMATA
void FiniteAutomata(char c) {

	if (dfaState == state_start) {
		dfaState == start(c);
	} else if (dfaState == state_f) {
		dfaState == f(c);
	} else if (dfaState == state_fr) {
		dfaState == fr(c);
	} else if (dfaState == state_fre) {
		dfaState = fre(c);
	} else if (dfaState == state_free) {
		dfaState = free(c);
	} else if (dfaState == state_free_) {
		dfaState = free_(c);
	}
	//state prototypes handling free_access
	else if (dfaState == state_free_a) {
		dfaState = free_a(c);
	} else if (dfaState == state_free_ac) {
		dfaState = free_ac(c);
	} else if (dfaState == state_free_acc) {
		dfaState = free_acc(c);
	} else if (dfaState == state_free_acce) {
		dfaState = free_acce(c);
	} else if (dfaState == state_free_access) {
		dfaState = free_access(c);
	}
	//state prototypes handling ree_software
	else if (dfaState == state_free_s) {
		dfaState = free_s(c);
	} else if (dfaState == state_free_so) {
		dfaState = free_so(c);
	} else if (dfaState == state_free_sof) {
		dfaState = free_sof(c);
	} else if (dfaState == state_free_soft) {
		dfaState = free_soft(c);
	} else if (dfaState == state_free_softw) {
		dfaState = free_softw(c);
	} else if (dfaState == state_free_softwa) {
		dfaState = free_softwa(c);
	} else if (dfaState == state_free_softwar) {
		dfaState = free_softwar(c);
	} else if (dfaState == state_free_software) {
		dfaState = free_software(c);
	}
	//state prototypes handling free_vacation
	else if (dfaState == state_free_v) {
		dfaState = free_v(c);
	} else if (dfaState == state_free_va) {
		dfaState = free_va(c);
	} else if (dfaState == state_free_vac) {
		dfaState = free_vac(c);
	} else if (dfaState == state_free_vaca) {
		dfaState = free_vaca(c);
	} else if (dfaState == state_free_vacat) {
		dfaState = free_vacat(c);
	} else if (dfaState == state_free_vacati) {
		dfaState = free_vacati(c);
	} else if (dfaState == state_free_vacatio) {
		dfaState = free_vacatio(c);
	} else if (dfaState == state_free_vacation) {
		dfaState = free_vacation(c);
	}
	//state prortypes free_trials
	else if (dfaState == state_free_t) {
		dfaState = free_t(c);
	} else if (dfaState == state_free_tr) {
		dfaState = free_tr(c);
	} else if (dfaState == state_free_tri) {
		dfaState = free_tri(c);
	} else if (dfaState == state_free_tria) {
		dfaState = free_tria(c);
	} else if (dfaState == state_free_trial) {
		dfaState = free_trial(c);
	} else if (dfaState == state_free_trials) {
		dfaState = free_trials(c);
	}
	//WIN STATES
	else if (dfaState == state_w) {
		dfaState = w(c);
	} else if (dfaState == state_wi) {
		dfaState = wi(c);
	} else if (dfaState == state_win) {
		dfaState = win(c);
	}
	//WINNINGS STATES
	else if (dfaState == state_winn) {
		dfaState = winn(c);
	} else if (dfaState == state_winni) {
		dfaState = winni(c);
	} else if (dfaState == state_winnin) {
		dfaState = winnin(c);
	} else if (dfaState == state_winning) {
		dfaState = winning(c);
	} else if (dfaState == state_winnings) {
		dfaState = winnings(c);
	}
	//WINNER(S) STATES
	else if (dfaState == state_winne) {
		dfaState = winne(c);
	} else if (dfaState == state_winner) {
		dfaState = winner(c);
	} else if (dfaState == state_winners) {
		dfaState = winners(c);
	}
	//FINAL
	else if (dfaState == state_final) {
...
enum State start(char c) {
	...
	if (c == 'f') {
		...
		dfaState = state_start;
		cout << "q" << state_start << endl; //no change
	}
	return dfaState;
}
enum State f(char c) {
	...
	if (c == 'r') {
		...
		dfaState = state_fr;
		cout << "q" << state_fr << endl;

	} else {
		dfaState = state_start;
		cout << "q" << state_start << endl;
	}
	return dfaState;
}
enum State fr(char c) {
...
enum State free_(char c) {
	...
	if (c == 'a') {
		dfaState = state_free_a;
		cout << "q" << state_free_a << endl;
	} else if (c == 's') {
		dfaState = state_free_s;
		cout << "q" << state_free_s << endl;
	} //move onto software test 
	else if (c == 'v') {
		dfaState = state_free_v;
		cout << "q" << state_free_v << endl;
	} // move onto vacation test
	else if (c == 't') {
		dfaState = state_free_t;
		cout << "q" << state_free_t << endl;
	} //move onto trials test
	else {
		dfaState = state_start;
		cout << "q" << state_start << endl;
	} //fail
...
enum State free_softwa(char c) {
	...
	if (c == 'r') {
		...
		cout << "q" << state_free_softwar << endl;
	} else {
		dfaState = state_start;
		cout << "q" << state_start << endl;
	}
...
enum State free_softwar(char c) {
	...
	if (c == 'e') {
		...
		cout << "q" << state_free_software << endl;
	} else {
		dfaState = state_start;
		cout << "q" << state_start << endl;
	}
...
enum State free_vacati(char c) {
	...
	if (c == 'o') {
		...
		cout << "q" << state_free_vacatio << endl;
	} else {
		dfaState = state_start;
		cout << "q" << state_start << endl;
	}
...
enum State free_vacatio(char c) {
	...
	if (c == 'n') {
		...
		cout << "q" << state_free_vacation << endl;
	} else {
		dfaState = state_start;
		cout << "q" << state_start << endl;
	}
...
enum State free_trials(char c) {
	...
	//FINAL PROMPT
	cout << "q" << "String 'free trials' accepted" << endl << endl;
	dfaState = state_final;
	cout << "q" << state_final << endl;
	return dfaState;
}
...
enum State winn(char c) {
	...
	if (c == 'i') {
		...
		cout << "q" << state_winni << endl;
	} else if (c == 'e') {
		dfaState = state_winne;
		cout << "q" << state_winne << endl;
	} else {
		dfaState = state_start;
	}
	return dfaState;
...
enum State final(char c) {
	printStateTransition(dfaState, c);
	cout << "qFINAL or q" << state_final << endl << endl;
	dfaState = state_final;
	return dfaState;
}