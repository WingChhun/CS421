...
void FiniteAutomata(char  c)
{
	...
	else if (dfaState == state_free_access){dfaState = free_access(c);}
	//state prototypes handling ree_software
	...
	else if (dfaState == state_free_software){dfaState = free_software(c);}
	//state prototypes handling free_vacation
	...
	else if (dfaState == state_free_vacation){dfaState = free_vacation(c);}
	//state prortypes free_trials
	...
	else if (dfaState == state_free_trials){dfaState = free_trials(c);}
	//WIN STATES
	else if (dfaState == state_w){dfaState = w(c);}
	else if (dfaState == state_wi){dfaState = wi(c);}
	else if (dfaState == state_win){dfaState = win(c);} 
	//WINNINGS STATES
	...
	else if (dfaState == state_winnings){dfaState = winnings(c);}
	//WINNER(S) STATES
	else if (dfaState == state_winne){dfaState = winne(c);}
	else if (dfaState == state_winner){dfaState = winner(c);}
	else if (dfaState == state_winners){dfaState = winners(c);} 
	//FINAL
...
enum State start(char c) {
	...
	if (c == 'f') {
		...
		dfaState = state_f;cout <<"q"<<state_f<<endl;
	}
	else if(c == 'w'){dfaState = state_w;cout <<"q"<<state_w<<endl;}
	else{dfaState = state_start;cout <<"q"<<state_start<<endl; //no change
	}
	return dfaState;
}
enum State f(char c) {
	...
	if (c == 'r') {
		...
		dfaState = state_fr;cout <<"q"<<state_fr<<endl;

	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
}
enum State fr(char c) {
	...
	if (c == 'e') {
		...
		dfaState = state_fre;cout <<"q"<<state_fre<<endl;
	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
}
enum State fre(char c) {
	...
	if (c == 'e') {
		...
		dfaState = state_free;cout <<"q"<<state_free<<endl;
	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
}
enum State free(char c) {
	...
	if (c == ' ') {
		dfaState = state_free_;cout <<"q"<<state_free_<<endl;
	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
}
enum State free_(char c) {
	...
	if (c =='a') {dfaState = state_free_a;cout <<"q"<<state_free_a<<endl;} 
	else if(c =='s'){dfaState =state_free_s;cout <<"q"<<state_free_s<<endl;} //move onto software test 
	else if(c =='v'){dfaState =state_free_v;cout <<"q"<<state_free_v<<endl;} // move onto vacation test
	else if(c =='t'){dfaState =state_free_t;cout <<"q"<<state_free_t<<endl;} //move onto trials test
	else {dfaState = state_start;cout <<"q"<<state_start<<endl;} //fail
	return dfaState;
}
//state prototypes handling free_access
enum State free_a(char c)
{
	...
	if (c =='c') {
		dfaState = state_free_ac;cout <<"q"<<state_free_ac<<endl;
	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
}
enum State free_ac(char c)
{
	...
	if (c =='c') {
		dfaState = state_free_acc;cout <<"q"<<state_free_acc<<endl;
	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
}
enum State free_acc(char c)
{
	...
	if (c =='e') {
		dfaState = state_free_acce;cout <<"q"<<state_free_acce<<endl;
	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
}
enum State free_acce(char c)
{
	...
	if (c =='s') {
		dfaState = state_free_access;cout <<"q"<<state_free_access<<endl;
	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
}
enum State free_access(char c)
{
	...
	dfaState = state_final;cout <<"q"<<state_final<<endl;
	return dfaState;
}
//state prototypes handling ree_software
enum State free_s(char c)
{
	...
	if (c =='o') {
		dfaState = state_free_so;cout <<"q"<<state_free_so<<endl;
	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
} 
enum State free_so(char c)
{
	...
	if (c =='f') {
		dfaState = state_free_sof;cout <<"q"<<state_free_sof<<endl;
	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
} 
enum State free_sof(char c)
{
	...
	if (c =='t') {
		dfaState = state_free_soft;cout <<"q"<<state_free_soft<<endl;
	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
} 
enum State free_soft(char c)
{
	...
	if (c =='w') {
		dfaState = state_free_softw;cout <<"q"<<state_free_softw<<endl;
	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
} 
enum State free_softw(char c)
{
	...
	if (c =='a') {
		dfaState = state_free_softwa;cout <<"q"<<state_free_softwa<<endl;
	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
}
enum State free_softwa(char c)
{
	...
	if (c =='r') {
		dfaState = state_free_softwar;cout <<"q"<<state_free_softwar<<endl;
	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
} 
enum State free_softwar(char c)
{
	...
	if (c =='e') {
		dfaState = state_free_software;cout <<"q"<<state_free_software<<endl;
	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
}
enum State free_software(char c)
...
enum State free_v(char c)
{
	...
	if (c =='a') {
		dfaState = state_free_va;cout <<"q"<<state_free_va<<endl;
	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
} 
enum State free_va(char c)
{
	...
	if (c =='c') {
		dfaState = state_free_vac;cout <<"q"<<state_free_vac<<endl;
	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
}
enum State free_vac(char c)
{
	...
	if (c =='a') {
		dfaState = state_free_vaca;cout <<"q"<<state_free_vaca<<endl;
	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
} 
enum State free_vaca(char c)
{
	...
	if (c =='t') {
		dfaState = state_free_vacat;cout <<"q"<<state_free_vacat<<endl;
	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
} 
enum State free_vacat(char c)
{
	...
	if (c =='i') {
		dfaState = state_free_vacati;cout <<"q"<<state_free_vacati<<endl;
	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
} 
enum State free_vacati(char c)
{
	...
	if (c =='o') {
		dfaState = state_free_vacatio;cout <<"q"<<state_free_vacatio<<endl;
	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
} 
enum State free_vacatio(char c)
{
	...
	if (c =='n') {
		dfaState = state_free_vacation;cout <<"q"<<state_free_vacation<<endl;
	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
}
enum State free_vacation(char c)
...
enum State free_t(char c)
{
	...
	if (c =='r') {
		dfaState = state_free_tr;cout <<"q"<<state_free_tr<<endl;
	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
} 
enum State free_tr(char c)
{
	...
	if (c =='i') {
		dfaState = state_free_tri;cout <<"q"<<state_free_tri<<endl;
	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
} 
enum State free_tri(char c)
{
	...
	if (c =='a') {
		dfaState = state_free_tria;cout <<"q"<<state_free_tria<<endl;
	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
} 
enum State free_tria(char c)
{
	...
	if (c =='l') {
		dfaState = state_free_trial;cout <<"q"<<state_free_trial<<endl;
	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
} 
enum State free_trial(char c)
{
	...
	if (c =='s') {
		dfaState = state_free_trials;cout <<"q"<<state_free_trials<<endl;
	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
} 
enum State free_trials(char c)
...
enum State w(char c)
{
	...
	if (c =='i') {
		dfaState = state_wi;cout <<"q"<<state_wi<<endl;
	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
}
enum State wi(char c)
{
	...
	if (c =='n') {
		dfaState = state_win;cout <<"q"<<state_win<<endl;
	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
}
enum State win(char c)
{
	...
	if (c =='n') {
		dfaState = state_winn;cout <<"q"<<state_winn<<endl;
	} else {dfaState = state_start;cout <<"q"<<state_start<<endl;}
	return dfaState;
} 
...
enum State winn(char c)
{
	...
	if (c =='i') {
		dfaState = state_winni;cout <<"q"<<state_winni<<endl;
	}
	else if(c =='e'){dfaState = state_winne;cout <<"q"<<state_winne<<endl;}
